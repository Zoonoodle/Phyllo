//
//  DataStoryView.swift
//  Phyllo - Momentum Redesign V2
//
//  Created on 1/31/25.
//
//  This is V2 of the Momentum tab redesign, focusing on telling
//  a compelling data story with chapters and narrative flow.
//

import SwiftUI

struct DataStoryView: View {
...
struct ChapterNavigator: View {
    @Binding var currentChapter: DataStoryView.StoryChapter
    let onChapterChange: () -> Void
    
    var body: some View {
        HStack(spacing: 12) {
            ForEach(DataStoryView.StoryChapter.allCases, id: \.self) { chapter in
                ChapterTab(
                    chapter: chapter,
...
struct ChapterTab: View {
    let chapter: DataStoryView.StoryChapter
    let isSelected: Bool
    let action: () -> Void
