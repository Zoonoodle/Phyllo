            ...
            VStack(spacing: 16) {
                ...
                HStack(spacing: 16) {
                    ...
                    if let score = scoreBreakdown {
                        VStack(spacing: 12) {
                            NutriSyncScoreMini(score: score.totalScore, trend: score.trend)
                                .opacity(animateContent ? 1 : 0)
...
struct TransformationMapView: View {
    ...
    var body: some View {
        ...
        .background(
            LinearGradient(
                colors: [Color.nutriSyncAccent.opacity(0.15), Color.nutriSyncAccent.opacity(0.05)],
                startPoint: .topLeading,
                endPoint: .bottomTrailing
            )
        )
...
struct PhaseMarker: View {
    ...
    var body: some View {
        VStack(spacing: 8) {
            ...
            Text(phase.days)
                .font(.system(size: 10))
                .foregroundColor(isSelected ? .white : .nutriSyncTextTertiary)
        }
        .offset(y: -16)
        .onTapGesture(perform: onTap)
    }
...
struct MissionStep: View {
    ...
    var body: some View {
        HStack(spacing: 16) {
            ZStack {
                Circle()
                    .stroke(isChecked ? Color.nutriSyncAccent : Color.white.opacity(0.3), lineWidth: 2)
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(title)
                    ...
                    .foregroundColor(isChecked ? .nutriSyncAccent : .white)
