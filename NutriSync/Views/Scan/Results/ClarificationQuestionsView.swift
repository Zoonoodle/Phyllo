...
struct ClarificationQuestionsView: View {
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                // If no questions available, skip clarification
                if questions.isEmpty {
                    VStack {
                        Spacer()
                        Text("Processing meal...")
                            .font(.system(size: 18, weight: .medium))
                            .foregroundColor(.white.opacity(0.7))
                        Spacer()
                    }
                    .onAppear {
                        // Immediately complete if no questions
                ...
                } // End of else statement for questions.isEmpty check
            }
        }
    ...
    private var questionSection: some View {
        Group {
            if let currentQuestion = currentQuestion {
                ...
                Text("No clarification questions available")
                    .font(.system(size: 16))
                    .foregroundColor(.white.opacity(0.5))
                    .padding(.horizontal, 20)
            }
    ...
    private func createMockMeal() -> LoggedMeal {
        ...
        let ingredients = [
            ...
            MealIngredient(name: "Olive Oil", quantity: 1, unit: "tbsp", foodGroup: .fatOil, calories: 120, protein: 0.0, carbs: 0.0, fat: 14.0),
            MealIngredient(name: "Seasoning Blend", quantity: 1, unit: "tsp", foodGroup: .condimentSauce, calories: 5, protein: 0.2, carbs: 1.0, fat: 0.1)
        ]
            ...
            HStack(spacing: 14) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    HStack(spacing: 8) {
                        ...
                        if option.isRecommended && option.note?.lowercased() == "assumed in base" {
                            // V2: Highlight the assumed option
                            Text("Assumed (default)")
                                .font(.system(size: 12, weight: .semibold))
