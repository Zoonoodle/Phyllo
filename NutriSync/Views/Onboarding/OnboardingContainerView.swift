                ...
                Text("Continue")
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 24)
                            .fill(canProceed ? Color.nutriSyncAccent : Color.white.opacity(0.2))
                    )
            }
            .disabled(!canProceed)
        }
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        HStack {
                            Text("Quiet hours")
                                ...
                                .foregroundColor(.white)
                            Spacer()
                            Toggle("", isOn: $data.quietHoursEnabled).labelsHidden().tint(.nutriSyncAccent)
                        }
    ...
    var body: some View {
        Button(action: action) {
            Image(systemName: symbol)
                ...
                .background(Circle().fill(Color.nutriSyncAccent))
        }
                    ...
                    Text("\(v)")
                        ...
                        .background(
                            RoundedRectangle(cornerRadius: 12)
                                .fill(v == selected ? Color.nutriSyncAccent : Color.white.opacity(0.06))
                        )
    ...
    var body: some View {
        Button(action: action) {
            HStack(spacing: 16) {
                ...
                Spacer()
                ZStack {
                    Circle().stroke(selected ? Color.nutriSyncAccent : Color.white.opacity(0.2), lineWidth: 2).frame(width: 22, height: 22)
                    if selected { Circle().fill(Color.nutriSyncAccent).frame(width: 14, height: 14) }
                }
            }
            .padding(16)
            .background(
                RoundedRectangle(cornerRadius: 16)
                    ...
                    .overlay(RoundedRectangle(cornerRadius: 16).stroke(selected ? Color.nutriSyncAccent.opacity(0.3) : Color.white.opacity(0.05), lineWidth: 1))
            )
        }
        .buttonStyle(PlainButtonStyle())
    }
    ...
    var body: some View {
        Button(action: action) {
            Text(title)
                ...
                .padding(.vertical, 8)
                .background(
                    Capsule().fill(selected ? Color.nutriSyncAccent : Color.white.opacity(0.08))
                )
        }
        .buttonStyle(PlainButtonStyle())
    }
